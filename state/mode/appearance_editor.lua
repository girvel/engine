...
local KINDS_OF_HAIR = {
  ...
  {
    codename = "short_hair_1",
    name = "Короткие (1)",
  },
  {
    codename = "short_hair_2",
    name = "Короткие (2)",
  },
  {
    codename = "short_hair_3",
    name = "Короткие (3)",
  },
  {
    codename = "long_hair",
  ...
  tk.start_window("center", "center", 780, 700)
    ...
    ui.start_font(24)
      ui.start_line()
        ui.selector()
        ui.text("Тип волос:")
        local hair_type_changed = ui.switch(KINDS_OF_HAIR, self.model, "hair_type")
      ui.finish_line()
    ui.finish_font()
    ui.finish_frame()

    if hair_type_changed then
      ...
      inventory.hair = hair_type ~= "none"
        and State:add(items_entities.hair(hair_type, "red"))
        or nil
    end
  tk.finish_window()
end
